...
From these results we can see that there are no significant differences between male and female pipefish in terms of standard length or torso depth. However, female dusky pipefish possess a significantly longer snout-vent length. For that measurement I am going to run a power test to ensure that we are confident in our results.
...
We have a power of over 0.9 or over 90% so we can be confident in our interpretation.
...
```
```{r save-to-file-fitness, eval=FALSE}
...
```{r lat-to-mate-matesucc, message=FALSE}
#Mating success
##Create the model
...
```{r lat-to-mate-reposucc, message=FALSE}
#Mating success
##Create the model
...
### Relationships with latency to mate
Similar to what was done with the males, we can look to see if the time spent in same-sex tanks in the lab impacted various components of female fitness.

```{r lat-to-mate-matesucc-fem, message=FALSE}
#Mating success
##Create the model
...
Similar to the males, there is little impact of the random effects for both collecting location and collecting month, but we can see that latency to trial has a negative but non-significant effect on mating success. 

```{r lat-to-mate-matesucc-fem-plot, fig.cap="_Scatterplot of the relationship between the number of mates achieved by a female and how long the female was in the lab prior to entering a trial.._", echo=FALSE}

plot(jitter(fem_succFL$lat_to_trial),
     jitter(fem_succFL$MatingSuccess),
     xlab = "Latency to Trial (Days)",
     ylab = "Mating Success",
     pch = 19,
     col = "#F1AE8B75",
     cex = 1.5)
abline(lm(fem_succFL$MatingSuccess ~ fem_succFL$lat_to_trial), lwd = 3, lty =2)

```

Now let's see if there is any relationship when we look at reproductive success (in terms of total eggs received) and the latency to the trial. Because we already accounted for the females who didn't mate in terms of mating success, I am going to remove them from this model. Therefore we will be looking at the impact on reproductive success on the individuals that were successfully able to obtain a mate.

```{r lat-to-mate-reposucc-fem, message=FALSE}
#Mating success
##Create the model
...
Again the random effects do not appear to be explaining any of the variance, however, there does appear to be a significant negative impact of latency to trial on the total number of eggs transferred by a female, similar to what we saw in the males.

```{r lat-to-mate-toteggs-fem-plot, fig.cap="_Scatterplot of the relationship between the total eggs transferred by a female and how long the female was in the lab prior to entering a trial._", echo=FALSE}

plot(jitter(mated_femFL$lat_to_trial),
     jitter(mated_femFL$totalEggs),
     xlab = "Latency to Trial (Days)",
     ylab = "Total Eggs Recieved",
     pch = 19,
     col = "#F1AE8B75",
     cex = 1.5)
abline(lm(mated_femFL$totalEggs ~ mated_femFL$lat_to_trial), lwd = 3, lty =2)
...
We can see that for male and female _S. floridae_ we don't see significant differences between the sexes for either the opportunity for selection or the opportunity for sexual selection. In both cases, there is a greater opportunity for selection compared to an opportunity for sexual selection.
...
I will be following the steps outlined in Arnold & Wade 1984 (Theory and Application):

  - Variables we are using which correspond to those found in **Table 1** of the Application paper:
  
     $W_1 = \frac{Mating Success_i}{\overline{Mating Success}}$ or **relative mating success**
    
    $W_1 W_2 = \frac{EggsTransferred_i}{\overline{EggsTransferred}}$ or **relative number of eggs transferred**
    
    $W_1 W_2 W_3 = \frac{EggsDeveloped_i}{\overline{EggsDeveloped}}$ or **relative number of eggs developed**

  
  - Variables we are using which correspond to equations 14 - 16 and A1 - A11 in the Theory paper:
  
    $w_1 = W_1$ or relative mating success
    
    $w_2 = \frac{W_1 W_2}{W_1}$ or relative number of eggs transferred **per mate**
    
    $w_3 = \frac{W_1 W_2 W_3}{W_1 W_2}$ or relative **proportion** of eggs developed
  ...
  - With those variables we could then calculate the opportunity for selection at each episode as:
...
```{r opp-select-episodes-figure, echo=FALSE, fig.cap="_Average opportunity for selection for the different episodes for male (purple) and female (green) S. floridae. Errorbars represent the 95% confidence intervals around the mean_", fig.width=10, fig.height=8, warning=FALSE}
...
From the table and the plot we can see that once again there are no significant differences in the selection between males and females.
...
I now want to look at any relationship that may exist between mating success and reproductive success for males and females. The Bateman gradient ($\beta_{SS}$) will be calculated. Historically, $\beta_{SS}$ is the slope of the weighted least-squares regression of relative reproductive success (number of offspring divided by the mean) on mating success. Here, I will be using a linear mixed effects model with the trial number included as a random effect rather than a least-squares regression. 
...
For both males and females there is a significant slope, meaning both sexes see increases in fitness with each additional mating. I am interested to run the model with the two datasets combined to see if there is an interaction of sex (i.e., are there differences between the Bateman gradient for males and females).
...
```{r bateman-no3, echo=FALSE, message=FALSE, fig.width=8, fig.height=8}
...
new_dat_fem <- data.frame(MatingSuccess = fem_seq,
                          trial = NA) 
preds_fem2 <- predict(fem_mod2, newdata = new_dat_fem, re.form = NA)
lines(fem_seq, preds_fem2,
      col = "#7fc97f",
      lwd = 3,
      lty = 2,
      xpd = FALSE)
...
new_dat_mal <- data.frame(MatingSuccess = mal_seq,
                          trial = NA) 
preds_mal <- predict(mal_mod2, newdata = new_dat_mal, re.form = NA)
lines(mal_seq, preds_mal,
      col = "#beaed4",
      lwd = 3,
      lty = 2,
...
```{r calc-bateman-gradient-nozero2, message=FALSE}
...
```{r plot-bateman-nozero2, echo=FALSE, fig.cap="_Relationship between reproductive success and mating success for female (green) and male (purple) _Syngnathus floridae_ who achieved at least one mate. Reproductive success is shown as relative fitness (i.e. number of offspring produced divided by the mean number of offspring produced). Relative fitness is calculated without the individuals who did not mate. Bateman's gradient is shown as the slope of a linear mixed effects model (dashed)._", fig.height=8, fig.width=8}
...
mal_seq <- seq(0, 2, length.out = 50)
new_dat_mal <- data.frame(MatingSuccess = mal_seq,
                          trial = NA) 
preds_mal <- predict(mal_mod3, newdata = new_dat_mal, re.form = NA)
lines(mal_seq, preds_mal,
      col = "#beaed4",
      lwd = 3,
      lty = 2,
...
We can do the same combination of datasets that we did earlier to see if there is a difference in the gradient between males and females.

```{r calc-bateman-all-nozero, message=FALSE}
...
We can see that when the non-mated individuals are included there is a steep slope (due to the zeros). Because I am interested in the post-mating processes, I am going to move forward without including the non-mated individuals (as they are accounted for in the Bateman gradient).

I will use a Spearman Rank correlation here as the data is not normally distributed (proportion and count data).
...
For both males and females there is no significant correlation between the number of mates and the proportion of the offspring which survive. However, both of the non-significant correlations are negative, meaning potentially if our sample size was higher we would be able to detect an effect.
