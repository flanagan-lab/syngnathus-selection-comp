...
```{r lat-to-mate-matesucc, eval=FALSE}
#Mating success
##Create the model
...
```{r lat-to-mate-reposucc, eval = FALSE}
#Mating success
##Create the model
...
```{r lat-to-mate-toteggs-plot, fig.cap="_Scatterplot of the relationship between the total eggs recieved by a male and how long the male was in the lab prir to entering a trial. Color of the points correspond to the location that the pipefish was collected from._", echo=FALSE}
...
datatable(cbind("Sex" = opp_average$Sex, 
                "Selection" = opp_average$Episode_sel, 
                "Average" = round(opp_average$Average, 2),
                "Upper CI" = round((opp_average$Average + opp_average$Interval), 2),
                "Lower CI" = round((opp_average$Average - opp_average$Interval), 2))) 
...
```{r opp-selection-females, eval = FALSE}
...
```{r opp-selection-males, eval = FALSE}
#Create a dataframe to store all of the intermediate values of fitness in
