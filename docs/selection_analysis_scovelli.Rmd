...
```
```{r write-reprod-succ, eval=FALSE}
...
For the Gulf pipefish, there are significant differences between males and females in terms of standard length, snout-vent length, and torso depth with females being significantly larger in all instances.

For these measurements I am going to run a power test to ensure that we are confident in our results.
...
There may be some correlation happening here, but it doesn't look particularly strong. Let's run some correlations tests to see what they say. As these data are normally distributed I will use the standard Pearson's correlation test.
...
The total eggs in mated females is not normally distributed so I will run a Spearman's rank correlation instead.

```{r eggs-fem-size-cor, echo=FALSE}
cor.test(mated_femSS$length, as.numeric(mated_femSS$totalEggs), method = "spearman")
cor.test(mated_femSS$depth_adj, as.numeric(mated_femSS$totalEggs), method = "spearman")
cor.test(mated_femSS$svl, as.numeric(mated_femSS$totalEggs), method = "spearman")
...
There were no significant differences in terms of the the length, SVL, or brood pouch of males who mated versus those who did not. However, males who mated had a significantly larger torso depth!
...
We can see that for female _S. scovelli_ there is a significant difference in the opportunity for sexual selection between males and females.
...
```{r opp-select-episodes-figure, echo=FALSE, fig.cap="_Average opportunity for selection for the different episodes for male (purple) and female (green) S. scovelli. Errorbars represent the 95% confidence intervals around the mean_", fig.width=10, fig.height=8, warning=FALSE}
...
x_labels_orig <- unique(opp_episodes_average$Episode_sel)
x_labels_display <- sapply(x_labels_orig, function(x) {
  
  ##COI with "given": e.g. "coi1_2given1" -> COI(1, 2|1)
  if (grepl("^coi(\\d+)_(\\d+)given(\\d+)$", x)) {
    m <- regmatches(x, regexec("^coi(\\d+)_(\\d+)given(\\d+)$", x))[[1]]
    return(bquote(COI(.(m[2])*","~.(m[3])*"|"*.(m[4]))))
  }
  
  ##COI without "given": e.g. "coi12_2" -> COI(12, 2)
  if (grepl("^coi(\\d+)_(\\d+)$", x)) {
    m <- regmatches(x, regexec("^coi(\\d+)_(\\d+)$", x))[[1]]
    return(bquote(COI(.(m[2])*","~.(m[3]))))
  }
  
  ##Diff: e.g. "diff_12" -> Diff(12)
  if (grepl("^diff_(\\d+)$", x)) {
    m <- regmatches(x, regexec("^diff_(\\d+)$", x))[[1]]
    return(bquote(Diff(.(m[2]))))
  }
  
  ##Subscript for "I_1", "I_2", etc.
  if (grepl("_", x)) {
    parts <- strsplit(x, "_")[[1]]
    return(bquote(.(parts[1])[.(parts[2])]))
  }
  
  ##Plain label, no transformation
  return(x)
  
})
  
#Set up empty plot with no axes
plot(NULL, xlim = c(0, 15),
     ylim = c(-2, 5),
     xlab = "", ylab = "",
     xaxt = "n", yaxt = "n",
     bty = "n")
  
#Add the x-axis, adjusting for the subscripted labels with mtext()
axis(1, at = 1:15, labels = FALSE, lwd = 2)
for (j in 1:15) {
  
  mtext(x_labels_display[[j]], side = 1, 
        at = j, line = 1.5, family = "serif", 
        cex = 1, las =2)
  
  }

##Add the y-axis
axis(2, cex.axis = 1.25, lwd = 2)
box(bty = "l", lwd = 2)  
  
#Plot points and error bars for each sex
for (j in 1:length(unique(opp_episodes_average$Sex))) {
    
  #Subset for the sex of interest
  sex_data <- opp_episodes_average[opp_episodes_average$Sex == unique(opp_episodes_average$Sex)[j], ]
  ...
  x_pos <- x_positions + (j - (length(unique(opp_episodes_average$Sex)) + 1)/2) * dodge_amount
    
  #Add the error bars
  arrows(x_pos, 
         sex_data$Average - sex_data$Interval,
         x_pos,
         sex_data$Average + sex_data$Interval,
         angle = 90, code = 3, length = 0.1, lwd = 2, col = "black")
    
  #Add the points
  points(x_pos, sex_data$Average,
         pch = 16,  
         col = sex_cols[unique(opp_episodes_average$Sex)[j]],
         cex = 1.5)
  }
...
mtext("Average Fitness Value", side = 2, outer = TRUE, line = 2, cex = 1.3)

#Add a legend
par(fig = c(0, 1, 0, 1), oma = c(5, 5, 0, 0),
    mar = c(3, 1, 0, 1), new = TRUE)
...
selection_colors <- c("I_1" = "#9FBCAA", "I_2" = "#F0B67F", "I_3" = "#CD96CD")

ggplot(opp_percents[opp_percents$Episode_sel %in% c("I_1", "I_2", "I_3"),], 
       aes(x = Sex, 
           y = Percent, 
           fill = Episode_sel)) +
...
Once we have the measures of relative fitness we can use them to run the linear mixed effects model for males and females separately.

```{r calc-bateman-gradient, message=FALSE}
...
```{r calc-bateman-all, message=FALSE}
...
fem_seq <- seq(0, 5, length.out = 100)
new_dat_fem <- data.frame(MatingSuccess = fem_seq,
                          trial = NA) 
preds_fem <- predict(fem_mod, newdata = new_dat_fem, re.form = NA)
lines(fem_seq, preds_fem,
      col = "#7fc97f",
      lwd = 3,
      lty = 2,
      xpd = FALSE)

mal_seq <- seq(0, 5, length.out = 100)
new_dat_mal <- data.frame(MatingSuccess = mal_seq,
                          trial = NA) 
preds_mal <- predict(mal_mod, newdata = new_dat_mal, re.form = NA)
lines(mal_seq, preds_mal,
      col = "#beaed4",
      lwd = 3,
      lty = 2,
      xpd = FALSE)
...
```{r calc-bateman-fem-nozero1, message=FALSE}
...
new_dat_fem <- data.frame(MatingSuccess = fem_seq,
                          trial = NA) 
preds_fem <- predict(fem_mod2, newdata = new_dat_fem, re.form = NA)
lines(fem_seq, preds_fem,
      col = "#7fc97f",
      lwd = 3,
      lty = 2,
...
Once we have the measures of relative fitness we can use them to run the linear mixed effects model.

```{r calc-bateman-gradient2, message=FALSE}
...
```{r plot-bateman, echo=FALSE, fig.cap="_Relationship between reproductive success and mating success for female _Syngnathus scovelli_ who achieved at least one mate. Reproductive success is shown as relative fitness (i.e. number of offspring produced divided by the mean number of offspring produced). Relative fitness is calculated without the individuals who did not mate. BBateman's gradient is shown as the slope of a linear mixed effects model (dashed)._", fig.height=8, fig.width=8}
...
fem_seq <- seq(0, 3, length.out = 50)
new_dat_fem <- data.frame(MatingSuccess = fem_seq,
                          trial = NA) 
preds_fem <- predict(fem_mod3, newdata = new_dat_fem, re.form = NA)
lines(fem_seq, preds_fem,
      col = "#7fc97f",
      lwd = 3,
      lty = 2,
