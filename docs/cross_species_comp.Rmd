...
for (i in 1:length(unique(opp_select_sub$species))) {
  ...
  species_data <- opp_select_sub[opp_select_sub$species == unique(opp_select_sub$species)[i], ]
  ...
  for (j in 1:length(unique(opp_select_sub$sex))) {
    ...
    sex_data <- species_data[species_data$sex == unique(opp_select_sub$sex)[j], ]
...
```{r opp-select-percent, echo = FALSE, fig.height=8, fig.width=8}
#Set the colors
...
for (i in 1:length(unique(opp_percents_sub$Species))) {
  ...
  for (j in 1:length(episodes)) {
    for (k in 1:length(sex_list)) {
      ...
      subset_val <- opp_percents_sub[opp_percents_sub$Species == sp & 
                              opp_percents_sub$Episode_sel == episodes[j] & 
                              opp_percents_sub$Sex == sex_list[k], "Percent"]
...
```{r fig-opp-select-assemble, message=FALSE, eval = FALSE}
...
The model we have chosen for the gradient is a linear mixed effects model where we have used the trial number as a random effect.

```{r fem-bateman, echo=FALSE, fig.height=8, fig.width=8, message = FALSE}
...
```{r s-m-prop-developed, echo= FALSE, fig.width=8, fig.height=8}
...
We can see a significant difference present in both _S. fuscus_ and _S. scovelli_. 
...
For consistency with the opportunity for selection metrics, we will instead use the number of developed eggs (rather than the total number of eggs stored) as our metric for reproductive success in the Bateman Gradient.
...
```{r fem-bateman-egg-dev, echo=FALSE, fig.height=8, fig.width=8, message = FALSE}
fem_bateman$species <- as.factor(fem_bateman$species)
fem_bateman$MatingSuccess <- as.numeric(fem_bateman$MatingSuccess)
fem_bateman$rel_repo_fitness <- as.numeric(fem_bateman$rel_repo_fitness)
...
slopes <- numeric(length(levels(fem_bateman$species)))
names(slopes) <- levels(fem_bateman$species)
...
plot(fem_bateman$rel_repo_fitness ~ fem_bateman$MatingSuccess,
     ylab = "", xlab = "",
     bty = "n", type = "n", axes = FALSE)

axis(1, pos = 0, lwd = 2, cex = 2, cex.axis = 1.5, las=1)
axis(2, pos = 0, at =seq(0, 8, by = 1), lwd = 2, cex = 2, cex.axis = 1.5, las = 1)

clip(0, 7.1, 0, 7.1)

for(species in levels(fem_bateman$species)){
  
  points(fem_bateman$rel_repo_fitness[fem_bateman$species == species] ~
           fem_bateman$MatingSuccess[fem_bateman$species == species],
         col = paste0(species_cols[species], "85"),
         pch = species_shapes[species],
         cex = 1.5)
...
for (sp in levels(fem_bateman$species)) {
  
  dat_sub <- subset(fem_bateman, species == sp)
  ...
  lines(x_seq, preds,
        col = species_cols[sp],
        lwd = 3,
        lty = which(levels(fem_bateman$species) %in% sp),
        xpd = FALSE)
...
Lastly, we can really look into the impact of additional mates by running the Bateman Gradient WITHOUT the individuals who didn't mate. For _S. floridae_ and _S. fuscus_ we will include both males and females as they mate multiply. For _S. scovelli_ only the females mate multiply.

We can start by re-calculating relative fitness metrics WITHOUT including individuals who did not mate for males and females:
...
With those new fitness values calculated we can visualize the data and re-run the model.

```{r bateman-nozeros, echo=FALSE, fig.height=4, fig.width=10, message = FALSE}
#Merge male and female datasets
fem_bateman_nozero$sex <- rep("F", times = nrow(fem_bateman_nozero))
mal_bateman_nozero$sex <- rep("M", times = nrow(mal_bateman_nozero))

bateman_nozero <- rbind(fem_bateman_nozero, mal_bateman_nozero)

#Convert variables
bateman_nozero$species <- as.factor(bateman_nozero$species)
bateman_nozero$sex <- as.factor(bateman_nozero$sex)

bateman_nozero$MatingSuccess <- as.numeric(bateman_nozero$MatingSuccess)
bateman_nozero$rel_repo_fitness <- as.numeric(bateman_nozero$rel_repo_fitness)

#Set colors and shapes for points
sex_cols <- c("F" = "#7fc97f", "M" = "#beaed4" )
species_shapes <- c("floridae" = 15,
                  "fuscus" = 16,
                  "scovelli" = 17)
...
#Create one plot per species
for (pipefish in unique(bateman_nozero$species)) {
  
  ##Pull out data for the species
  tmp_species <- bateman_nozero[bateman_nozero$species == pipefish, ]
  
  #Set the base plot
  plot(tmp_species$rel_repo_fitness ~ tmp_species$MatingSuccess,
       ylab = "", xlab = "", ylim = c(0, 2.5), xlim = c(0.4, 2.4),
       bty = "n", type = "n", axes = FALSE)
  
  #Add an x- and y-axis to the plot
  axis(1, at = seq(-1, 2.5, 0.2),
       lwd = 2, cex = 2, cex.axis = 1.5, las=1)
  axis(2, at = seq(-1, 3.5, 0.5),
       lwd = 2, cex = 2, cex.axis = 1.5, las = 1)
  
  #Loop through the sexes and add points and lines to the plot
  for(sex in unique(tmp_species$sex)){
    
    #Subset data for sex
    tmp_sex <- tmp_species[tmp_species$sex == sex,]
    
    #Add the points
    points(tmp_sex$rel_repo_fitness ~ tmp_sex$MatingSuccess,
           col = paste0(sex_cols[sex], "85"),
           pch = species_shapes[pipefish],
           cex = 1.5)
    
    #Create vector of x-values
    x_seq <- seq(min(tmp_sex$MatingSuccess), 
                 max(tmp_sex$MatingSuccess),
                 length.out = 100)
    
    #Run the model
    mod <- lmer(rel_repo_fitness ~ MatingSuccess + (1 | trial), data = tmp_sex,
                REML = TRUE)
    ...
    #Add the predicted values as a line to the plot
    lines(x_seq, preds,
        col = sex_cols[sex],
        lwd = 3,
        lty = which(levels(bateman_nozero$sex) %in% sex),
        xpd = FALSE)
    
    }
  
  mtext(paste0("S.", pipefish), 3, cex = 1.25, line = -1.5)
  
}

mtext("Relative Mating Success", 1 , cex = 1.5, line = 2, outer = TRUE)
mtext("Relative Reproductive Success", 2, cex = 1.5, line = 1.2, outer = TRUE)
...
To begin we can compare the selection differentials calculated for males and females across the species in a similar manner to the opportunity for selection above. The data I will be using for this includes the average selection differentials across the trials for each episode along side the upper and lower percentile-based confidence intervals. Both the averages and the confidence intervals were generated in the document `bootstrapping_selection_analysis.Rmd`.

I am first going to plot the average selection differential value with the confidence intervals as error bars. 

```{r select-diff-ave-CI, echo = FALSE, fig.height=8, fig.width=10}

#pdf("figs/select_diff_aveCI.pdf", width = 10, height=8)
...
#Subset data to only keep the episodes of selection which we are interested in
select_diff_sub <- select_diff[select_diff$episode_sel %in% c("s1_prime", "s2_prime", 
                                                              "s3_prime", "s123_prime"),]
...
#Define the desired order for x-axis
x_order <- c("s1_prime", "s2_prime", "s3_prime", "s123_prime")
x_labels_orig <- x_order[x_order %in% unique(select_diff_sub$episode_sel)]
...
x_labels_display <- sapply(x_labels_orig, function(x) {
  ...
  } else if (grepl("_prime$", x)) {

    # Extract the number after "s" and before "_prime"
    num <- sub("s(.+)_prime", "\\1", x)
    return(bquote(s*"'"[.(num)]))
...
#Loop through the three species and plot data
for (i in 1:length(unique(select_diff_sub$species))) {
  
  #Subset for the species of interest
  species_data <- select_diff_sub[select_diff_sub$species == 
                                    unique(select_diff_sub$species)[i], ]
  
  #Set up empty plot with no axes
  plot(NULL, xlim = c(0, 4.5),
       ylim = c(-1, 1),
       xlab = "", ylab = "",
       xaxt = "n", yaxt = "n",
       bty = "n")
  ...
  #Plot points and error bars for each sex
  for (j in 1:length(unique(select_diff_sub$sex))) {
    
    #Subset for the sex of interest
    sex_data <- species_data[species_data$sex == unique(select_diff_sub$sex)[j], ]
    
    #Match x positions to categorical positions
    x_positions <- match(sex_data$episode_sel, x_labels_orig)
    
    #Apply position adjustment to the points
    x_pos <- x_positions + (j - (length(unique(select_diff_sub$sex)) + 1)/2) * dodge_amount
    ...
    points(x_pos, sex_data$average_cal,
           pch = 15 + i,  
           col = sex_cols[unique(select_diff_sub$sex)[j]],
           cex = 3)
  }
...
#Add overall axis labels
mtext("Standardized selection differential", side = 1, outer = TRUE, line = 2.3, cex = 1.3)
mtext("Average Value", side = 2, outer = TRUE, line = 2.5, cex = 1.3)

#Add a legend
par(fig = c(0, 1, 0, 1), oma = c(5, 5, 2, 0),
    mar = c(3, 1, 2, 1), new = TRUE)
plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
...
I am going to try running a multinomial logistic regression where I will use a trait of interest to predict mating success females in the three species. I will start with snout-vent-length as the trait of interest.

```{r log-reg-model-svl, message=FALSE}
library(nnet)
...
Now that we have the models, we can use them to predict the probability of having a mating success of X at each trait value.

```{r log-reg-vis, echo=FALSE, fig.width=10, fig.height=5}
par(mfrow = c(1,3))
#S. floridae
##Create prediction range
svl_range_FL <- seq(min(FL_fem_fitness$svl), 
                    max(FL_fem_fitness$svl), length.out = 100)
predicted_probs_FL <- predict(model_FL, 
                              newdata = data.frame(svl = svl_range_FL), 
                              type = "probs")

##Plot the predicted probabilities for each category
plot(svl_range_FL, predicted_probs_FL[,1], type = "l", col = "#2F97C1", lwd = 2,
     ylim = c(0, 1),
     xlab = "Snout-Vent Length (mm)",
     ylab = "Predicted Probability",
     main = "S. floridae")
lines(svl_range_FL, predicted_probs_FL[,2], col = "#FAC748", lwd = 2)
lines(svl_range_FL, predicted_probs_FL[,3], col = "#C05746", lwd = 2)
lines(svl_range_FL, predicted_probs_FL[,4], col = "#7fc97f", lwd = 2)

legend("topright", 
       legend = c("0 matings", "1 mating", "2 matings", "3 matings"),
       col = c("#2F97C1", "#FAC748", "#C05746", "#7fc97f"), 
       lwd = 2, 
       bty = "n")

#S. fuscus
##Create prediction range
svl_range_FU <- seq(min(FU_fem_fitness$svl), 
                    max(FU_fem_fitness$svl), length.out = 100)
predicted_probs_FU <- predict(model_FU, 
                              newdata = data.frame(svl = svl_range_FU), 
                              type = "probs")

##Plot the predicted probabilities for each category
plot(svl_range_FU, predicted_probs_FU[,1], type = "l", col = "#2F97C1", lwd = 2,
     ylim = c(0, 1),
     xlab = "Snout-Vent Length (mm)",
     ylab = "Predicted Probability",
     main = "S. fuscus")
lines(svl_range_FU, predicted_probs_FU[,2], col = "#FAC748", lwd = 2)
lines(svl_range_FU, predicted_probs_FU[,3], col = "#C05746", lwd = 2)

legend("topright", 
       legend = c("0 matings", "1 mating", "2 matings"),
       col = c("#2F97C1", "#FAC748", "#C05746"), 
       lwd = 2, 
       bty = "n")

#S. scovelli
##Create prediction range
svl_range_SS <- seq(min(SS_fem_fitness$svl), 
                    max(SS_fem_fitness$svl), length.out = 100)
predicted_probs_SS <- predict(model_SS, 
                              newdata = data.frame(svl = svl_range_SS), 
                              type = "probs")

##Plot the predicted probabilities for each category
plot(svl_range_SS, predicted_probs_SS[,1], type = "l", col = "#2F97C1", lwd = 2,
     ylim = c(0, 1),
     xlab = "Snout-Vent Length (mm)",
     ylab = "Predicted Probability",
     main = "S. scovelli")
lines(svl_range_SS, predicted_probs_SS[,2], col = "#FAC748", lwd = 2)
lines(svl_range_SS, predicted_probs_SS[,3], col = "#C05746", lwd = 2)
lines(svl_range_SS, predicted_probs_SS[,4], col = "#7fc97f", lwd = 2)
...
The only species we can see any clear patterns for is _S. scovelli_, when a female reaches a certain snout-vent length, her chances for securing a mate increase considerably.

I will re-run the same thing now using torso depth as the trait of interest.

```{r log-reg-model-depth}
##Convert mating success and run the model
FL_fem_fitness$MatingSuccess <- factor(FL_fem_fitness$MatingSuccess)
...
```{r log-reg-vis-depth, echo=FALSE, fig.width=10, fig.height=5}
par(mfrow = c(1,3))
#S. floridae
##Create prediction range
depth_range_FL <- seq(min(FL_fem_fitness$depth), 
                    max(FL_fem_fitness$depth), length.out = 100)
predicted_probs_FL <- predict(model_FL, 
                              newdata = data.frame(depth = depth_range_FL), 
                              type = "probs")

##Plot the predicted probabilities for each category
plot(depth_range_FL, predicted_probs_FL[,1], type = "l", col = "#2F97C1", lwd = 2,
     ylim = c(0, 1),
     xlab = "Torso Depth (mm)",
     ylab = "Predicted Probability",
     main = "S. floridae")
lines(depth_range_FL, predicted_probs_FL[,2], col = "#FAC748", lwd = 2)
lines(depth_range_FL, predicted_probs_FL[,3], col = "#C05746", lwd = 2)
lines(depth_range_FL, predicted_probs_FL[,4], col = "#7fc97f", lwd = 2)
...
#S. fuscus
##Create prediction range
depth_range_FU <- seq(min(FU_fem_fitness$depth), 
                    max(FU_fem_fitness$depth), length.out = 100)
predicted_probs_FU <- predict(model_FU, 
                              newdata = data.frame(depth = depth_range_FU), 
                              type = "probs")

##Plot the predicted probabilities for each category
plot(depth_range_FU, predicted_probs_FU[,1], type = "l", col = "#2F97C1", lwd = 2,
     ylim = c(0, 1),
     xlab = "Torso Depth (mm)",
     ylab = "Predicted Probability",
     main = "S. fuscus")
lines(depth_range_FU, predicted_probs_FU[,2], col = "#FAC748", lwd = 2)
lines(depth_range_FU, predicted_probs_FU[,3], col = "#C05746", lwd = 2)
...
#S. scovelli
##Create prediction range
depth_range_SS <- seq(min(SS_fem_fitness$depth), 
                    max(SS_fem_fitness$depth), length.out = 100)
predicted_probs_SS <- predict(model_SS, 
                              newdata = data.frame(depth = depth_range_SS), 
                              type = "probs")

##Plot the predicted probabilities for each category
plot(depth_range_SS, predicted_probs_SS[,1], type = "l", col = "#2F97C1", lwd = 2,
     ylim = c(0, 1),
     xlab = "Torso Depth (mm)",
     ylab = "Predicted Probability",
     main = "S. scovelli")
lines(depth_range_SS, predicted_probs_SS[,2], col = "#FAC748", lwd = 2)
lines(depth_range_SS, predicted_probs_SS[,3], col = "#C05746", lwd = 2)
lines(depth_range_SS, predicted_probs_SS[,4], col = "#7fc97f", lwd = 2)
...
For torso depth, we can start to see some interesting patterns forming for all three species, there is a higher probability that at least one mating will be secured if you have a deeper torso.
