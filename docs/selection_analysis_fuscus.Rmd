...
I will be looking at total standard length (mm, measured from the tip of the snout to the tip of the caudal fin), snout-vent length (mm, measured from the tip of the snout to the urogenital opening), and torso depth (mm). To look at these differences I will be performing t-tests between males and females. First, I need to see if assumptions are met, i.e. variances are equal and data is normally distributed.
...
Based on the assumption testing above, I will run a Wilcoxon test for standard length, a two sample t-test for snout-vent length, and a Wilcoxon test for torso depth (adjusted).
...
For standard length and snout-vent length I am going to run a power test to ensure that we are confident in our results.
...
There may be some correlation happening here, but it doesn't look particularly strong. Let's run some correlations tests to see what they say. As the variables are normally distributed I will use the standard Pearson's correlation.
...
Within the mated females, length and depth are not normally distributed. As a result I will use the Spearman's Rank correlation for those two.

```{r eggs-fem-size-cor, echo=FALSE}
cor.test(mated_femFU$length, as.numeric(mated_femFU$totalEggs), method = "spearman")
cor.test(mated_femFU$depth_adj, as.numeric(mated_femFU$totalEggs), method = "spearman")
cor.test(mated_femFU$svl, as.numeric(mated_femFU$totalEggs))

```

There is no sig. correlation between length, svl, or depth and the number of eggs transferred, however, all three correlations are positive.
...
var.test(mal_succFU$depth_adj ~ mal_succFU$preg_status) #not equal
...
normalTest(mal_succFU$depth_adj, method = "da") #not normal
...
wilcox.test(mal_succFU$depth_adj ~ mal_succFU$preg_status) #not sig.
...
normalTest(fem_succFU$depth_adj, method = "da") #normal
...
t.test(fem_succFU$depth_adj ~ fem_succFU$mated,
       var.equal = FALSE) #not sig.
...
```{r opp-select-episodes-figure, echo=FALSE, fig.cap="_Average opportunity for selection for the different episodes for male (purple) and female (green) S. fuscus. Errorbars represent the 95% confidence intervals around the mean_", fig.width=10, fig.height=8, warning=FALSE}
...
Once we have the measures of relative fitness we can use them to run the mixed effects model for males and females separately.
...
fem_seq <- seq(0, 7, length.out = 100)
...
mal_seq <- seq(0, 7, length.out = 100)
...
```{r bateman-no3, echo=FALSE, message=FALSE}
